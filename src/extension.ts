...
export function activate(context: vscode.ExtensionContext) {
  ...
  const launchCommand = vscode.commands.registerCommand(
    "rad-debugger-communication.launch",
    async () => {
...
function findBreakpointIRIndex(
  arr: BreakpointIR[],
  value: BreakpointIR,
): number {
  let i = 0;
...
function refreshConfig() {
  ...
  target_path_is_workspace_relative =
    config.get("targetPathIsWorkspaceRelative") ?? true;
...
async function runRadSession(targetPath: string) {
  ...
  if (!isRunning) {
    ...
    let sanityIterMax = Math.max(
      (wait_for_process_timeout / waitTimeMs) * 2,
      1,
    );
    ...
    while (true) {
      ...
      const timeDiff = Math.max(
        newDate.getMilliseconds() - prevDate.getMilliseconds(),
        0,
      );
      prevDate = newDate;
  ...
  if (!isRunning) {
    console.log(
      "failed to spawn raddbg instance, or failed to detect the instance is running.",
    );
    return;
  }
  ...
  for (const bp of vscode.debug.breakpoints) {
    if (bp instanceof vscode.SourceBreakpoint) {
      ...
      const add_command =
        rad_debugger_path + " --ipc add_breakpoint " + breakpoint.path;
  ...
  if (auto_run_target) {
    ...
    const enableTargetCommand =
      rad_debugger_path + " --ipc enable_target" + targetIdentifier;
...
async function run(commandString: string) {
  try {
    await execAsync(commandString, { cwd: cwd });
  } catch (error) {
  ...
  implements vscode.DebugAdapterDescriptorFactory
{
  createDebugAdapterDescriptor(
    ...
    console.log(
      "Creating inline spoof debug adapter for session:",
      session.name,
    );
    return new vscode.DebugAdapterInlineImplementation(
      new RadDebuggerSpoofSession(),
    );
  }
}
...
class RadDebuggerSpoofSession extends DebugSession {
  ...
  protected launchRequest(
    ...
    // set the cwd to be the launch.json one
    cwd = (args as any).cwd;
